...
export const SMART_PROFILES = {
  duel: {
    ...
    holdHighCardsWhileTalon: false,// ВЫКЛЮЧЕН: придерживание Q+ стоило 12.6 п.п.
    avoidBurningBigTrump: true,    // лучше взять, чем спалить козырного K/A ради мелочи
    finishOffWeakOpponent: true,   // добивать соперника картой, которую он не бьёт
    takeWhenTableUnbeatable: true, // брать сразу, если весь стол не отбить
    exactEndgame: true,            // точный счёт, когда прикуп пуст
    dumpPairs: false,              // выключен по результатам перекалибровки (−5.1 п.п. на 2x24 с ним)
    exactEndgameSolver: true,      // ПОЛНЫЙ перебор концовки дуэли
    attackByPressure: false,
    probabilisticTake: false,
  },
  small: {
    holdTrumpsWhileTalon: true,
    holdHighCardsWhileTalon: false,
    avoidBurningBigTrump: false,   // на 4 игроках без него лучше (roadmap 1.4)
...
/** Обратная совместимость для скриптов и тестов, ссылающихся на SMART_PROFILE. */
export const SMART_PROFILE = SMART_PROFILES.duel;

export function pickProfileName(rules) {
  const numPlayers = rules && typeof rules.numPlayers === 'number' ? rules.numPlayers : 2;
  if (numPlayers <= 2) return 'duel';
  if (numPlayers <= 4) return 'small';
  return 'large';
}
