...
export class SmartBot {
  ...
  _tryExactSolver(state, playerId, legalActions) {
    ...
    if (!this.profile.exactEndgameSolver) {
      stats.disabled = (stats.disabled || 0) + 1;
      return null;
    }
    if (!this.tracker) {
      stats.noTracker = (stats.noTracker || 0) + 1;
      return null;
    }
    if (!canSolve(state, this.tracker, playerId)) {
      stats.unapplicable = (stats.unapplicable || 0) + 1;
      return null;
    }

    const res = solveFromState(state, this.tracker, playerId, this.solverOptions);
    if (!res) {
      stats.nullResult = (stats.nullResult || 0) + 1;
      return null;
    }
    stats.calls++;
    stats.nodes += res.nodes || 0;
    stats.ms += res.ms || 0;
    ...
    const sameSet = res.legal && legalActions && res.legal.length === legalActions.length
      && res.legal.every((r) => legalActions.some((a) => sameEndgameAction(a, r)));
    const chosen = legalActions && legalActions.find((a) => sameEndgameAction(a, res.action));
