...
export const SMART_PROFILE = {
  ...
  attackByPressure: true,        // ходить/подкидывать картой с наибольшим шансом, что соперник НЕ отобьётся
                                 // (`bestAttackByPressure`: давление минус нормированная цена карты),
                                 // вместо «просто самой дешёвой». Цифры A/B — в отчёте issue #49.
  probabilisticTake: true,       // «брать или отбиваться» по ожидаемой цене: сравниваем ожидаемую цену
...
// Ниже этого шанса отбиться защита считается безнадёжной (сравнение идёт по ожидаемой цене,
// это лишь отсечка для «совсем плохо»). Выведено из смысла величины, а не подобрано порогом:
// при p < 0.5 «отбиться» проигрывает «взять» уже по одному только матожиданию потерь.
const DEFENSE_HOPELESS_P = 0.5;
